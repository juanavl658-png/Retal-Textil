...
export function calcularTodo(inputs: InputsCorte): ResultadosCorte {
  ...
  const areaTotalTodasCapas = redondear(inputs.largoTelaM * inputs.anchoTelaM * inputs.lineasTela, 3)
  const retalGeneradoM2 = calcularRetalGenerado(
    areaTotalTodasCapas,
    areaConsumidaM2
  )

  const pctRetal = calcularPctRetal(retalGeneradoM2, areaTotalTodasCapas)
  ...
  return {
    areaPrendaM2,
    areaTotalDisponibleM2: areaTotalTodasCapas,
    prendasPorLinea,
    totalPrendas,
    areaConsumidaM2,
    retalGeneradoM2,
